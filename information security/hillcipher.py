...
Original file is located at
    https://colab.research.google.com/drive/1nFd3N1boT8CpDOK8XH0985m0VjSBYUtC
"""
...
def encryption(m):
    ...
    # Replacing spaces
    m = m.replace(" ", "")
    # Asking for key and generating encryption matrix
    C = make_key()
    # Appending zero if the messsage isn't divisble by 2
    len_check = len(m) % 2 == 0
    if not len_check:
        m += "0"
    # Converting the message matrix into corresponding integers
    ...
    for i in range(m_len):
        ...
        # Mod with 26 and add 65 to get back to the A-Z range in ascii
        integer = int(row_0 % 26 + 65)
        # Change back to chr type and add to text
        en_m += chr(integer)
        # Repeating same process for the second column
...
def decryption(en_m):
    # Asking for key and getting encryption matrix
    C = make_key()
    # Inverse matrix
